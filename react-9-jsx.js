...
function ReactComponent(props) {
  /*
  React에서 말하는 props 매개변수는 '반드시' 객체 타입이어야 한다는 약속이 있으므로 매개 '변수' 보다는 무엇이 들어있을지 모르는 여러개의 값의 뭉치 '객체'로 보아야 합니다
  */
  ...
  리엑트에서는 전개연산자와 구조분해할당을 매우 애용하므로, 익숙해지는 것이 좋습니다
  */
  const {tagName, ...otherProps } = props;

  // return은 사실상 DOM API를 사용하는 것이 아니라, 리엑트가 제공하는 createElement 함수를 사용합니다
  // 이것을 JSX 문법이라고 부르며, bable이 이것을 createElement 함수로 변환해 줍니다
